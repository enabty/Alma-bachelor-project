...
def init_training_data_from_npy(pos_path, neg_path, linnear_aug=False, augment_factor=5):
    ...
    if linnear_aug:
        ...
        fits_pos = np.array([crop_middle_100x100(file) for file in fits_pos if file.shape == (400, 400)])
        fits_neg = np.array([crop_middle_100x100(file) for file in fits_neg if file.shape == (400, 400)])
