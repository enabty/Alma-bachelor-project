...
def linear_transformation(ret_image):
    ...
    if random.getrandbits(1): ret_image = np.fliplr(ret_image)
    if random.getrandbits(1): ret_image = np.flipud(ret_image)
    ...
    return ret_image[y:y+100, x:x+100]
